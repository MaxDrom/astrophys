...
print(f"Int {filter}: {np.min(y)}")
plt.figure(figsize=(20, 10))
plt.plot(x, y)
plt.xlabel(r'$r$ in seconds')
plt.ylabel(r'$m(r)$')
plt.savefig(filter+'/' + filter+'_slice_'+"I(R)"+ '.png')  
centmag = dcircle_I(1, center[0], center[1], 10, 100, result_data)
centmag=-2.5*np.log(centmag)
centmag = centmag-26.74+26*2.5*math.log(3.828)
print(f"{filter}: {centmag}")
...
plt.xlabel(r'$r$ in seconds')
plt.ylabel(r'$m(r)$')
plt.savefig(filter+'/' + filter+'_slice_'+"dI(R)"+ '.png')  